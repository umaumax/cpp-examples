...
def gcno_filter(input_filepath, output_filepath, verbose=False):
    ...
    with open(output_filepath, mode='w') as output_file:
        ...
        for tmp_i in range(max(len(lines) - 1, 0)):
            ...
            if match_function:
                ...
                # NOTE: _xxx -> __xxx
                if re_function_replacer.match(func_name):
...
def main():
    ...
    gcno_filter(args.input_filepath, args.output_filepath, args.verbose)
